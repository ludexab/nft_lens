...
const App = () => {
  ...
  const ViewResult = ({name,symbol,token_address,contract_type}:{name:string,symbol:string,token_address:string,contract_type:string})=>{

      return(
        <div className='result'>
          <p className='result'>
          <span className='text-xl font-bold'>Token name:</span> <strong className='text-xl font-bold'>{name}</strong> <br />
  ...
  const ExploreResult = ({name,amount,symbol,owner,token_address,contract_type,token_uri}:{name:string,amount:string,owner:string,token_uri:string,symbol:string,token_address:string,contract_type:string})=>{
  ...
  const viewNft = async () =>{
    const address = document.getElementById('wallet-address') as HTMLInputElement;
    const network = document.getElementById('wallet-network') as HTMLInputElement;

    const options = {
      method: 'GET',
      url:`https://deep-index.moralis.io/api/v2/${address?.value}/nft/collections`,
      params: {chain: `${network?.value.toString()}`},
      headers: {accept: 'application/json', 'X-API-Key': import.meta.env.VITE_MORALIS_KEY}
    }
    try{
  ...
  return (
    ...
    <header>
      <div className='header pb-3 gap-5'>
        ...
        {connected? (<div className='button-div'>
          <button onClick={connectWallet} className='connect-wallet'>{`${account.slice(0,5)}...${account.slice(-4)}`}</button>
    ...
    <div className="main-div w-full">
        <p>
          Search for NFTs you own, Explore NFTs !!
        </p>
      <div className='filter-div w-[80%] grid grid-cols-1 lg:grid-cols-2 md:grid-cols-1'>
        <div className='filter-id grid grid-cols-1 justify-items-center sm:w-full'>
          ...
          <input className='input' id='wallet-address' type="text" placeholder='Wallet Address' defaultValue={account} />
          ...
          <button className='view-wallet' onClick={viewNft}>VIEW</button>
          {view.length !=0 && <hr className='hr' />}
          {view.map((obj:any, i)=>{
            return(
              <ViewResult key={i} name={obj.name} symbol={obj.symbol} token_address={obj.token_address} contract_type={obj.contract_type}/>
            )
          })}
        </div>
        <div className='filter-wallet grid grid-cols-1 justify-items-center sm:w-full'>
          ...
          <input className='input' id='id-address' type="text" placeholder='NFT Contract Address' />
          <input className='input' id='token-id' type="number" placeholder='Token ID' />
          ...
          <button className='view-id' onClick={exploreNft}>EXPLORE</button>
          {explore.length !=0 && <hr className='hr' />}
            {explore.map((obj:any, i)=>{
              return(
                <ExploreResult key={i} name={obj.name} symbol={obj.symbol} token_address={obj.token_address} contract_type={obj.contract_type} owner={obj.owner_of} amount={obj.amount} token_uri={obj.token_uri}/>
              )
            })}
        </div>
      </div>
